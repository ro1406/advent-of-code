...
def dfs_iterative(i,j,grid):
    ...
    q = deque([(i, j, 'R', 0)])  # Start at S facing East (Right)
    visited = {}
    best_paths=[]
    ...
    while q:
        x, y, curr_dir, curr_score = q.popleft()
        ...
        for new_dir, (dx, dy) in directions.items():
            ...
            q.append((nx, ny, new_dir, new_score))
...
# print(grid)
best_score = dfs_iterative(len(grid)-2,1,grid)
print(f"{best_score=}")
